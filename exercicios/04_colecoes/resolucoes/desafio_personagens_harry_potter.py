...
# 1. Liste todos os personagens da Lufa-lufa (Hufflepuff)
# for p in personagens:
#   if p['house'] == 'Hufflepuff':
#     print(p['name'])

# 2. Liste todos os personagens que são bruxos (wizard = True)
# for p in personagens:
#   if p['wizard']:
#     print(p['name'])

# 3. Liste todos os personagens que são trouxas (wizard = False)
# for p in personagens:
#   if p['wizard'] is False:
#     print(p['name'])

# 4. Faça um personagem aprender uma das magias possíveis (utilize a coleção 'magias' para encontrar a magia)
for magia in magias:
    if magia['name'] == 'Accio':
        for p in personagens:
            if p['name'] == 'Harry Potter':
                p['spells'] = [magia]
                print(p)
                break
    else:
        print('Magia não existe!')
        break


def aprender_magia(nome_magia, nome_personagem):
    for magia in magias:
        if magia != nome_magia.title():
            print(f'{nome_magia.title()} não existe!')
            break
        else:
            for personagem in personagens:
                if personagem['name'] is not nome_personagem:
                    print(f'{nome_personagem.title()} não é um personagem existente!')
                    break


aprender_magia('Pó de Piripimpim', 'HP')
aprender_magia('Accio', 'HP')
